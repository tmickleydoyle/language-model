#!/usr/bin/env python3
"""Train 125M logic-focused model with chain-of-thought data.

Optimized for reasoning tasks with:
- 24 layers, 512 embedding dim (~125M params)
- 2048 context for multi-step reasoning
- Lower dropout for precision

Supports resuming from checkpoint:
    python train_logic_model.py --resume logic_model/best_model.pth --epochs 500 --target-loss 1.5
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train logic-focused model')
    parser.add_argument('--resume', type=str, default=None,
                        help='Path to checkpoint to resume from')
    parser.add_argument('--epochs', type=int, default=300,
                        help='Total epochs to train')
    parser.add_argument('--target-loss', type=float, default=1.5,
                        help='Target validation loss to stop at')
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logic_training.log')
    ]
)
...
class LogicDataset:
    """Dataset for logic data with loss masks."""
...
def main():
    ...
    print("=" * 60)
    print("Logic-Focused 125M Model Training")
    print("=" * 60)

    # Configuration
    TOKENIZER_PATH = PROJECT_ROOT / "models" / "tokenizer_instruct"
    DATA_DIR = PROJECT_ROOT / "data" / "tokenized_logic"
    OUTPUT_DIR = PROJECT_ROOT / "logic_model"

    # Model hyperparameters - ~77M params (proven stable on MPS)
    EPOCHS = args.epochs
    BATCH_SIZE = 4
    CONTEXT_SIZE = 1024
    EMBEDDING_DIM = 512
    ...
    # Muon-specific hyperparameters - lower LR for better convergence
    MUON_LR = 0.0005
    ADAMW_LR = 5e-5
    MOMENTUM = 0.95
    NS_STEPS = 5
    ...
    WARMUP_RATIO = 0.05
    DROPOUT = 0.03  # Lower dropout for better fit
    ...
    train_dataset = LogicDataset(DATA_DIR / "train.pt", CONTEXT_SIZE)
    val_dataset = LogicDataset(DATA_DIR / "val.pt", CONTEXT_SIZE)
    ...
    if args.resume:
        ...
        logger.info(f"  Resumed from epoch {start_epoch}, val_loss: {best_val_loss:.4f}, global_step: {global_step}")

    # Training loop
    logger.info("=" * 60)
    logger.info("Starting training with chain-of-thought data")
    ...
    # Calculate total steps - limit batches per epoch for faster iterations
