#!/usr/bin/env python3
"""Pre-tokenize logic data with loss masks.
...
DATA_FILE = PROJECT_ROOT / "data" / "logic_cot" / "data.jsonl"
TOKENIZER_PATH = PROJECT_ROOT / "models" / "tokenizer_instruct"
OUTPUT_DIR = PROJECT_ROOT / "data" / "tokenized_logic"
...
def main():
    print("Pre-tokenizing logic data with loss masks...")
    ...
    print(f"\nSequence lengths:")
    print(f"  Min: {min(train_lens)}, Max: {max(train_lens)}, Avg: {sum(train_lens)/len(train_lens):.1f}")
