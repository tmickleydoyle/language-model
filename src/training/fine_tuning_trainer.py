...
This module provides specialized training functionality for fine-tuning pre-trained models
on instruction-following datasets including:
...
- Memory-efficient training with automatic garbage collection
- Robust error handling and recovery mechanisms
- Detailed metrics tracking and logging
- Support for validation and evaluation on instruction datasets
"""
...
class FineTuningTrainer(Trainer):
    ...
    Raises:
        ...
        TypeError: If model or dataset types are incorrect
    ...
    def __init__(
        ...
        # Store instruction-specific components
        ...
        self._log_trainer_initialization()
    ...
    def _validate_initialization_parameters(
        ...
        Args:
            ...
            instruction_dataset: Training dataset to validate
            val_dataset: Optional validation dataset to validate
            base_model_path: Optional base model path to validate

        Raises:
            TypeError: If parameter types are incorrect
            FileNotFoundError: If base_model_path doesn't exist
        """
        self._validate_model_parameter(model)
        self._validate_config_parameter(config)
        self._validate_dataset_parameters(instruction_dataset, val_dataset)
        self._validate_base_model_path(base_model_path)

    def _validate_model_parameter(self, model: GPTLanguageModel) -> None:
    ...
    def _validate_dataset_parameters(
        ...
        Raises:
            TypeError: If dataset types are incorrect
        """
    ...
    def _validate_base_model_path(self, base_model_path: Optional[Union[str, Path]]) -> None:
        """Validate base model path if provided.
        ...
        if base_model_path is not None:
            ...
            _check_file_existence(base_model_path, "Base model path")

    def _setup_trainer_components(self) -> None:
        """Setup all trainer components with validation and error handling."""
        self._validate_trainer_configuration()
        self._setup_training_data_components()
        self._initialize_comprehensive_metrics()

    def _log_trainer_initialization(self) -> None:
        ...
        self._log_memory_information()

    def _log_memory_information(self) -> None:
        """Log memory information if available."""
        if torch.cuda.is_available():
            memory_allocated = torch.cuda.memory_allocated() / 1024**2  # MB
            memory_reserved = torch.cuda.memory_reserved() / 1024**2  # MB
            logger.info(
                f"Initial GPU memory - Allocated: {memory_allocated:.1f}MB, "
                f"Reserved: {memory_reserved:.1f}MB"
            )

    def _validate_trainer_configuration(self) -> None:
        ...
        """
        self._validate_required_config_attributes()
        self._validate_positive_config_values()
        self._validate_optional_config_values()

        logger.debug("Configuration validation passed")

    def _validate_required_config_attributes(self) -> None:
    ...
    def _validate_positive_config_values(self) -> None:
        """Validate that configuration values are positive where required.
    ...
    def _validate_optional_config_values(self) -> None:
        """Validate optional configuration values and device settings.

        Raises:
            ValueError: If optional values are invalid
            TypeError: If device type is invalid
        """
        # Validate gradient clipping
        if hasattr(self.config, "grad_clip") and self.config.grad_clip < 0:
            raise ValueError(f"grad_clip must be non-negative, got {self.config.grad_clip}")
        ...
        if not isinstance(self.device, (str, torch.device)):
            raise TypeError(f"device must be str or torch.device, got {type(self.device)}")

    def _initialize_comprehensive_metrics(self) -> None:
    ...
    def _memory_management_context(self):
        ...
        Yields:
            None: Context for memory-managed operations
        """
        try:
    ...
    def _check_memory_usage(self) -> Dict[str, float]:
        ...
        if torch.cuda.is_available():
            memory_stats.update(self._get_gpu_memory_stats())
            self._warn_high_memory_usage(memory_stats)
    ...
    def _get_gpu_memory_stats(self) -> Dict[str, float]:
        ...
        Returns:
            Dictionary containing GPU memory stats in MB
        """
        return {
    ...
    def _warn_high_memory_usage(self, memory_stats: Dict[str, float]) -> None:
        """Warn if memory usage is too high.

        Args:
            memory_stats: Memory statistics dictionary
        """
        allocated_mb = memory_stats.get("gpu_allocated_mb", 0)
        threshold_mb = MIN_MEMORY_THRESHOLD_MB * 50  # 5GB threshold

        if allocated_mb > threshold_mb:
            logger.warning(f"High GPU memory usage: {allocated_mb:.1f}MB")
    ...
    def _log_training_progress(
        ...
        Args:
            ...
            step_metrics: Metrics for current step
        """
        if step % 10 == 0:  # Log every 10 steps
            memory_stats = self._check_memory_usage()
            gradient_norm = self._compute_gradient_norm()

            log_parts = self._build_progress_log_parts(
                epoch, step, step_metrics, gradient_norm, memory_stats
            )

            logger.debug(" | ".join(log_parts))

    def _build_progress_log_parts(
        self,
        epoch: int,
        step: int,
        step_metrics: Dict[str, float],
        ...
        """Build log parts for training progress.
        ...
        if memory_stats:
            gpu_mem = memory_stats.get('gpu_allocated_mb', 0)
            log_parts.append(f"GPU Mem: {gpu_mem:.0f}MB")

        return log_parts

    def _setup_training_data_components(self) -> None:
    ...
    def _create_learning_rate_scheduler(self) -> None:
        """Create learning rate scheduler for fine-tuning."""
        self.scheduler = self._create_scheduler_by_type()

    def _create_scheduler_by_type(self) -> Optional[torch.optim.lr_scheduler._LRScheduler]:
    ...
    def _load_base_model(self) -> None:
        """Load base model if path is specified.
    ...
    def _load_base_model_weights(self) -> None:
        ...
        """
        from ..utils.helpers import _check_file_existence
        _check_file_existence(self.base_model_path, "Base model")
        ...
        try:
            ...
            filtered_state_dict = self._filter_compatible_weights(state_dict)
            self._load_filtered_weights(filtered_state_dict)
    ...
    def _filter_compatible_weights(
        self,
        state_dict: Dict[str, torch.Tensor]
    ) -> Dict[str, torch.Tensor]:
        """Filter out incompatible weights to handle size mismatches gracefully.

        Args:
            state_dict: Original state dictionary

        Returns:
            Filtered state dictionary with compatible weights only
        """
        model_state_dict = self.model.state_dict()
        filtered_state_dict = {}

        for key, value in state_dict.items():
            if self._is_weight_compatible(key, value, model_state_dict):
                filtered_state_dict[key] = value
            else:
                self._log_incompatible_weight(key, value, model_state_dict)

        return filtered_state_dict

    def _is_weight_compatible(
        self,
        key: str,
        value: torch.Tensor,
        model_state_dict: Dict[str, torch.Tensor]
    ) -> bool:
        """Check if weight is compatible with model.
        ...
        Returns:
            True if weight is compatible
        """
        return (key in model_state_dict and
                value.shape == model_state_dict[key].shape)

    def _log_incompatible_weight(
        self,
        key: str,
        value: torch.Tensor,
        model_state_dict: Dict[str, torch.Tensor]
    ) -> None:
        """Log information about incompatible weights.
    ...
    def _load_filtered_weights(self, filtered_state_dict: Dict[str, torch.Tensor]) -> None:
        """Load filtered weights into model.

        Args:
            filtered_state_dict: Dictionary of compatible weights
        """
    ...
    def _compute_instruction_loss(
        ...
        # Add bounds checking to ensure target tokens don't exceed vocabulary size
        # This prevents "Target X is out of bounds" errors in cross-entropy computation
        valid_mask = target_ids_flat < vocab_size
        if not valid_mask.all():
            # Log warning about out-of-vocabulary tokens
            invalid_indices = (target_ids_flat >= vocab_size).nonzero().flatten()
            max_invalid = target_ids_flat[invalid_indices].max().item()
            logger.warning(
                f"Found {len(invalid_indices)} target tokens >= vocab_size ({vocab_size}). "
                f"Max invalid token: {max_invalid}. Clipping to vocab_size-1."
            )
            # Clip invalid tokens to vocab_size - 1 (typically the unknown token)
            target_ids_flat = torch.clamp(target_ids_flat, 0, vocab_size - 1)
    ...
    def load_checkpoint(self, checkpoint_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        from ..utils.helpers import _check_file_existence
        _check_file_existence(checkpoint_path, "Checkpoint")
